...
function normalizeArticle(input: any, index: number): PostInput {
  ...
  return {
    ...
    excerpt: String(input?.excerpt || input?.metaDescription || '').trim() || 'A sharp MakeMoneyOrDie essay about money, leverage, attention, and online opportunity.',
...
function fallbackArticles(): PostInput[] {
  const now = new Date();
  const stamp = now.toISOString().replace(/[-:T.Z]/g, '').slice(0, 12);
  return ['Affiliate Marketing', 'AI Side Hustles', 'Digital Products'].map((topic, index) => ({
    title: `${topic} playbook ${stamp}`,
    slug: `${makeSlug(topic)}-playbook-${stamp}-${index + 1}`,
    excerpt: 'A locally generated placeholder article. Configure OpenRouter to generate production content.',
    contentHtml:
      `<h2>Starter</h2><p>This placeholder appears because OPENROUTER_API_KEY is not configured. Replace it by setting production AI credentials.</p><h2>Detailed Analysis</h2><p>Use this topic as a starting point for a provocative, practical MakeMoneyOrDie essay about online opportunity, behavioral economics, and building leverage.</p><h2>About the Author</h2><p>Andrew Nicklson writes about money, attention, leverage, and the uncomfortable math of building your own future.</p>`,
    tags: [topic.toLowerCase(), 'online income'],
  }));
}
...
export async function generateArticlesWithOpenRouter(count = 3): Promise<PostInput[]> {
  ...
  if (!apiKey) {
    return fallbackArticles().slice(0, articleCount);
  }
